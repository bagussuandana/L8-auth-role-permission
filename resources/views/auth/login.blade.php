...
<div class="flex h-screen">
    <div class="card max-w-md md:rounded-lg m-auto">
        <form class="bg-white shadow-xl mb-4 flex flex-wrap justify-center py-10" action="{{route('login')}}" method="POST">
            @csrf
            <div class="w-full bg-teal-500 p-5 text-white">
                ...
                @enderror
                <button class="block w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 focus:outline-none focus:shadow-outline" type="submit">Login</button>

                <label class="block text-sm my-4" for="remember">
                    <input type="checkbox" name="remember" id="remember" /> Remember me
                </label>

                <a class="block w-full text-sm text-right text-white hover:text-gray-300" href="#">Forgot Password?</a>
            </div>
            <div class="w-full p-6 flex flex-col justify-between">
                ...
                <a class="block w-full mb-8 text-sm text-center text-blue-600 hover:text-blue-700" href="{{route('register')}}">Don't have an account? Register Now!</a>
